...
public class ScreenConnecting extends Screen
{
  ...
  Button back = new Button(540, 1400, 900, 200, "Back", new Runnable()
  {
    @Override
    public void run()
    {
  ...
  public void draw()
  {
    ...
    App.app.drawer.setFontSize(100);
    ...
    if (!this.finished)
    {
      App.app.drawer.drawText(540, 600, this.text);

      App.app.drawer.setColor(255, 0, 0);

      drawSpinny(100, 4, 0.3, 4 * 45 * size, 4 * 1 * size);
      drawSpinny(99, 3, 0.5, 4 * 30 * size2, 4 * 0.75 * size2);
      drawSpinny(100, 2, 0.7, 4 * 15 * size3, 4 * 0.5 * size3);
    }
    else
    {
      App.app.drawer.drawText(540, 600, this.text);
